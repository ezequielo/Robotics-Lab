...
void callback(const sensor_msgs::Image::ConstPtr& colorMsg, const sensor_msgs::Image::ConstPtr& depthMsg)
{
   cv_bridge::CvImageConstPtr bColor, bDepth;
	...
	cv::Mat mask;
	cv::threshold(depth,mask,1.0,255.0,cv::THRESH_BINARY_INV);

	mask.convertTo(mask,CV_8U);

	//This function will track points of interest
	//The points of interest will be only tracked where indicated by the mask
	//The mask image must be a 8-bit integer matrix with non-zero values in the region of interest.
	//In this example we will only track points closer than 1 meter (see threshold above)
	trackFeatures(color,mask);
